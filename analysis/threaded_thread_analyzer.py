...
THREAD_COUNTS = range(1, 31)  # Test from 1 to 20 threads
T_ARG = 20  # arg -t to pass to the program
MAX_PARALLEL_JOBS = len(THREAD_COUNTS)  # Number of tests to run in parallel
...
def parse_approx2_log(filename):
    ...
    for entry in entries:
        ...
        results[test_file] = {"length": path_length, "path": path, "time": elapsed}
...
def run_test_with_threads(thread_count):
    ...
    try:
        ...
        for bash_exe in bash_executables:
            try:
                ...
                test_result = subprocess.run(
                    [bash_exe, "--version"], capture_output=True, timeout=5
                )
        ...
        if bash_cmd is None:
            print(
                f"Thread count {thread_count}: Error - Could not find bash executable."
            )
        ...
        if result.returncode != 0:
            print(
                f"Thread count {thread_count}: Warning - Non-zero exit code: {result.returncode}"
            )
        ...
        if log_file.exists():
            ...
            print(
                f"Thread count {thread_count}: Completed - {len(results)} test results"
            )
...
def analyze_results(all_results):
    ...
    for test_file in test_files:
        ...
        for results in all_results.values():
            if (
                results
                and test_file in results
                and results[test_file]["length"] is not None
            ):
    ...
    analysis = {
        ...
        "per_file": defaultdict(
            lambda: {"threads": [], "lengths": [], "percentages": []}
        ),
        "summary": {},
...
def create_visualizations(analysis, output_dir):
    ...
    ax1.plot(
        threads, avg_percentages, marker="o", color="blue", linewidth=2, markersize=8
    )
    ...
    ax2.set_title(
        "Best Solutions Found vs Thread Count", fontsize=14, fontweight="bold"
    )
    ...
    if len(analysis["per_file"]) > 1 and len(analysis["per_file"]) <= 10:
        ...
        ax.set_title(
            "Path Length vs Thread Count (Per Test File)",
            fontsize=14,
            fontweight="bold",
        )
        ...
        plt.tight_layout()
        plt.savefig(
            output_dir / "per_file_comparison.png", dpi=300, bbox_inches="tight"
        )
...
def print_summary(analysis):
    ...
    best_percentage_thread = max(
        analysis["per_thread"].items(), key=lambda x: x[1]["avg_percentage"]
    )
    best_solutions_thread = max(
        analysis["per_thread"].items(), key=lambda x: x[1]["best_count"]
    )
    ...
    print(
        f"Most Best Solutions: {best_solutions_thread[0]} threads "
...
def main():
    ...
    if results_json.exists():
        results_json.unlink()
        print(f"\nCleared previous results from {results_json}")
    ...
    with ProcessPoolExecutor(max_workers=MAX_PARALLEL_JOBS) as executor:
        # Submit all jobs
        future_to_thread = {
            executor.submit(run_test_with_threads, tc): tc for tc in THREAD_COUNTS
        }
