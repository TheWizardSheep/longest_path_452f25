...
parser.add_argument(
    "--file", type=str, help="Input file path or directory (if omitted, read from STDIN)"
)
...
def parse_graph(file_path=None):
    global graph
    graph = defaultdict(dict)

    # Determine which file to read
    current_file = file_path if file_path is not None else input_via_file

    # Read in input as a file instead of STDIN
    if current_file is not None:
        if os.path.isdir(current_file):
            # If it's a directory, process all files in it
            files = sorted([f for f in os.listdir(current_file) if os.path.isfile(os.path.join(current_file, f))])
